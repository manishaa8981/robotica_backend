const Operating = require("../models/operatingModel");
const { responseHandler } = require("../helper/responseHandler");
const fs = require("fs");
const path = require("path");

// Manage main section (no image here now)
const manageOperating = async (req, res) => {
  try {
    const { mainTitle } = req.body;

    let section = await Operating.findOne();
    if (section) {
      ...
      const newSection = new Operating({ mainTitle, items: [] });
...
// Get section
const getOperating = async (req, res) => {
  try {
    const section = await Operating.findOne();
    if (!section)
...
const addItem = async (req, res) => {
  try {
    ...
    const section = await Operating.findById(mainId);
    if (!section)
